...
for (w in 1:nrow(df_start)) {
  ...
  if(file.exists("df_complex_energy.csv")){
    ...
    if(file.exists("df_RMSD_all.csv")){
      ...
      if (dir.exists("din")) {system(command = paste0("rm -r ",paste0(part_fin,"din/")),ignore.stdout=T,wait = T)}
      ...
      v_groups<-list.files(paste0("din"))
      ...
      if(nrow(df_groups)>0){
        ...
        if (dir.exists(paste0("fin_str/"))) {system(command = paste0("rm -r ",paste0(part_fin,"fin_str/")),ignore.stdout=T,wait = T)}
        if (!dir.exists(paste0("fin_str/"))) {dir.create(paste0("fin_str/"))}
        j<-1
