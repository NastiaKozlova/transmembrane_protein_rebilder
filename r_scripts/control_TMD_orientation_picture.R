...
df_start<-df_start%>%mutate(first_part_start=min(first_part_start))
df_start<-df_start%>%mutate(first_part_finish=max(first_part_finish))
df_start<-df_start%>%mutate(second_part_start=min(second_part_start))
df_start<-df_start%>%mutate(second_part_finish=max(second_part_finish))
...
df_start$name<-"start"
df_start<-df_start%>%select(name,first_part_start,first_part_finish, 
                            second_part_start, second_part_finish)
df_start<-unique(df_start)
...
for (i in 1:nrow(df_start)) {
    ...
    pdb_name<-paste0(part_start,"start/",df_start$name[i],".pdb")
    ...
    write.csv(df_tcl,paste0("make_picture_tcl_center/",df_start$name[i],".tcl"),row.names = F)
}

df_tcl<-read.csv(paste0("make_picture_tcl_center/",df_start$name[1],".tcl"),stringsAsFactors = F)
i<-2
if(nrow(df_start)>1){
    for (i in 2:nrow(df_start)) {
        df_tcl_add<-read.csv(paste0("make_picture_tcl_center/",df_start$name[i],".tcl"),stringsAsFactors = F)
