...
if (!dir.exists(paste0(part_start,"results/first_part/pictures/"))){dir.create(paste0(part_start,"results/first_part/pictures/"))}
...
for (i in 1:nrow(df_start)) {
  df_start$script[i]<-paste0("cd ",part_start,"programs/dist/\n", part_start,"programs/dist/bin/Ring -i ",
                             part_start,"results/first_part/structure/",df_start$name[i],"/",df_start$name[i],"_",df_start$group_number[i],".pdb > ",
                             part_start,"results/first_part/ring2/",df_start$name[i],"_",df_start$group_number[i],".txt\n",
                             "rm ",part_start,"results/first_part/structure/",df_start$name[i],"/",df_start$name[i],"_",df_start$group_number[i],".pdb_fasta*\n",
                             "rm ",part_start,"results/first_part/structure/",df_start$name[i],"/",df_start$name[i],"_",df_start$group_number[i],".pdb_modified\n")
}
df_script<-df_start%>%select(script)
write.table(df_script,paste0(part_start,"results/first_part/ring2.txt"), row.names = F,quote=F,col.names = F)
system(paste0("chmod +x ",part_start,"results/first_part/ring2.txt"),ignore.stdout=T,wait = T)
system(paste0(part_start,"results/first_part/ring2.txt"),ignore.stdout=T,wait = T)

i<-1
for (i in 1:nrow(df_start)) {
  if (file.exists(paste0("results/first_part/ring2/",df_start$name[i],"_",df_start$group_number[i],".txt"))){
    if (!file.exists(paste0("results/first_part/ring/",df_start$name[i],"_",df_start$group_number[i],".txt"))){
      df_ring<-read_delim(paste0("results/first_part/ring2/",df_start$name[i],"_",df_start$group_number[i],".txt"), delim = "\t", skip = 11)
      ...
      write.csv(df_ring,paste0("results/first_part/ring/",df_start$name[i],"_",df_start$group_number[i],".txt"),row.names = F)
    }
  }
}
i<-1
...
for (i in 1:nrow(df_start)) {
  if (file.exists(paste0("results/first_part/ring/",df_start$name[i],"_",df_start$group_number[i],".txt"))){
    ...
    df_ring<-unique(df_ring)
    write.csv(df_ring,paste0("results/first_part/TMD_interactions/",df_start$name[i],"_",df_start$group_number[i],".txt"),row.names = F)
  }
}
