...
if (!dir.exists(paste0(part_start,"results/first_part/plot/"))){dir.create(paste0(part_start,"results/first_part/plot/"))}
i<-1
...
for (i in 1:nrow(df_start)) {
  ...
  if(nrow(df_ring)>1){
    ...
    ggsave(p,filename = paste0("plot/",df_start$name[i],"_",df_start$group_number[i],".png"), width = 20, height = 20, units = c("cm"), dpi = 200 ) 
  }
}
